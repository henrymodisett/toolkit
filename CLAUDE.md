...
- **A downstream project is not fixed from here, whatever its state.** A project still on committed copies of the old scripts keeps working because no stripped release reaches it; an adopted one carries its own declarations and is reached by the installed tool. Read which it is from the project itself — `touchstone adopt --check` for its declarations, `touchstone policy status` for what GitHub enforces — never from a list in this file, and never from whether `policy/github/consumers/` holds an entry: that directory carries a file only for a repository whose policy varies from the canonical one, so absence there proves nothing. Naming it here was the same mistake one paragraph later, caught in review. Re-adoption is always a separate, tracked decision.
